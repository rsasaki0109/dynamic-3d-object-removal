...
- 連続 checked-in 版は、repo に per-frame box が入っていないため temporal consistency で transient outlier を拾い、そこから自動提案した `auto transient boxes` で sampled box-removal preview と `Peak replay` / `Peak burst` を作っています
